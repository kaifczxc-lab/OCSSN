...
if tris_new is not None:
    ...
    print("none")
"""
# At some point, the author discovered a shortcut in the dataset that could be characterized as follows: mesh_tris mesh size almost perfectly correlates with class ; 
# it was discovered using the LogisticRegression model, the shortcut was fixed, but this function remains as a diagnostic that it no longer exists
def logisticRegression(dataset_list):
    sizes = np.array([len(t) for t, lab in dataset_list]).reshape(-1, 1)
    labels = np.array([lab for t, lab in dataset_list])
    model = LogisticRegression()
    model.fit(sizes, labels)
    acc = model.score(sizes, labels)
    print(f"acc: {acc:.2%} (need to be +-50-60%)")
    return acc
...
# dataset generation
def dataset(k, t_min, t_max, n=None, seed=None, factor=20, verbose=True):
    if seed is not None:
        random.seed(seed)
    if n is None:
        n = k // len(spheres)
    ...
    for b in spheres:
        ...
        gen = chain(b, 0, n * factor, tag=tag, stats=stats) # generation g=0 (spheres)
        # protection against found shortcut (mesh_tris mesh size almost perfectly correlates with class)
        candidates = [(tris, lab) for tris, lab in gen if t_min <= len(tris) <= t_max]
        if len(candidates) < n:
            print(f"tag={tag} ; {len(candidates)}/{n} candidates in size range {t_min}, {t_max} after {n*factor}")
        random.shuffle(candidates)
        out += candidates[:n]
    gen_t = chain(torus(3, 3), 1, k * factor, tag="torus", stats=stats) # generation g=1 (torus)
    candidates_t = [(tris, lab) for tris, lab in gen_t if t_min <= len(tris) <= t_max]
    if len(candidates_t) < k:
        print(f"tag=torus ; {len(candidates_t)}/{k} candidates in size range {t_min}, {t_max} after {k*factor}")
    random.shuffle(candidates_t)
    out += candidates_t[:k]
    global last_stats
    last_stats = stats
    if verbose:
        dataset_logging(stats, seed)
        sphere_sizes = [len(t) for t, lab in out if lab == 0]
        torus_sizes = [len(t) for t, lab in out if lab == 1]
        print(f"sphere F: min={min(sphere_sizes)} max={max(sphere_sizes)} mean={sum(sphere_sizes)/len(sphere_sizes):.1f} n={len(sphere_sizes)}")
        print(f"torus F: min={min(torus_sizes)} max={max(torus_sizes)} mean={sum(torus_sizes)/len(torus_sizes):.1f} n={len(torus_sizes)}")
    return out
...
class OCSSN(torch.nn.Module):
    ...
    def forward(self, m, return_h=False, step=None):
        ...
        return z
set_seed(main_seed)
data = dataset(K, t_min=18, t_max=44, seed=main_seed)
data_val = dataset(K, t_min=18, t_max=44, seed=main_seed + 1)

logisticRegression(data)
logisticRegression(data_val)
